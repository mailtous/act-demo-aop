...
public final class HelloClassMetaInfo extends DestroyableBase {
    ...
    private C.List<HelloMethodMetaInfo> actions = C.newList();
    // actionLookup index job method by method name
    private C.Map<String, HelloMethodMetaInfo> actionLookup = null;
    ...
    public HelloClassMetaInfo superType(Type type) {
        superType = type;
        return this;
    }
    ...
    public HelloClassMetaInfo isJob(boolean b) {
        isJob = b;
        return this;
    }

    public HelloClassMetaInfo addAction(HelloMethodMetaInfo info) {
        actions.add(info);
        return this;
    }

    public HelloMethodMetaInfo action(String name) {
        if (null == actionLookup) {
            for (HelloMethodMetaInfo act : actions) {
    ...
    private void buildActionLookup() {
        C.Map<String, HelloMethodMetaInfo> lookup = C.newMap();
        for (HelloMethodMetaInfo act : actions) {
    ...
    private static final C.Set<Class<? extends Annotation>> ACTION_ANNOTATION_TYPES = C.set(Hello.class);
